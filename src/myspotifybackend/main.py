...
class Application:
    ...
    async def cleanJson(responseIn):  # Cleans Track Object
        ...
        return await Application.cleanDict(jsonIn)
    ...
    async def getAccessToken(refresh_token, basic, TOKEN_ENDPOINT):
        ...
        response = await asyncio.to_thread(
            requests.post, TOKEN_ENDPOINT, params=data, headers=headers
        )
    ...
    class nowPlayingResource:
        ...
        async def nowPlayingJSON(self):
            ...
            accessToken = await getAccessToken(
                self.refresh_token, basic, self.tokenEndpoint
            )
        ...
        async def getSongObject(self, songID):
            ...
            accessToken = await Application.getAccessToken(
                self.refresh_token, basic, self.tokenEndpoint
            )
        ...
        async def on_get(self, req, resp):
            ...
            if nowPlaying == "Error":
                ...
                isCurrentlyPlayingTask = asyncio.create_task(
                    self.currentlyPlayingBool(nowPlaying)
                )
                jsonCleanedTask = asyncio.create_task(
                    Application.cleanJson(nowPlaying)
                )  #
                ...
                if await isCurrentlyPlayingTask:
                    ...
                    # print(songObject)
                    textToRespond.update(
                        {"totalDuration": int(songObject.json()["duration_ms"] / 1000)}
                    )  #
    ...
    class topTracksResource:
        ...
        async def on_get(self, req, resp):
            ...
            tasks = [
                asyncio.create_task(Application.cleanDict(song))
                for song in getTopTracks.json()["items"]
            ]
